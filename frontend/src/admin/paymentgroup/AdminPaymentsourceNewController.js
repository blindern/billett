...
angular
  ...
  .controller(
    ...
    function ($modalInstance, $scope, paymentgroup, AdminPaymentsource, Page) {
      ...
      ctrl.cashitems = [1, 5, 10, 20, 50, 100, 200, 500, 1000]
      ctrl.cashinputs = {}
      ctrl.cashvalues = {}
      ...
      function setCashTitle() {
        ctrl.paymentsource.title =
          paymentgroup.eventgroup.paymentsources_data["cash_prefix"] || ""
      }

      function parseInputs() {
        ctrl.othervalue =
          ctrl.other === "" ? 0 : evaluate(ctrl.other.replace(",", "."))
      ...
      function isInputOk() {
        if (isNaN(ctrl.othervalue)) {
          Page.toast(
            "Feltet for " +
              (ctrl.paymentsource.type == "cash" ? "annet " : "") +
        ...
        if (ctrl.paymentsource.type == "cash") {
          ...
          if (s == 0) {
            Page.toast("Du kan ikke registrere en tom opptelling", {
              class: "danger",
            })
            return false
          }
        ...
        function errorNeg() {
          Page.toast("Du kan ikke registrere en negativ telling", {
            class: "danger",
          })
        }
      }
      ...
      function updateDataField() {
        ...
        if (ctrl.othervalue != 0)
          ctrl.paymentsource.data["other"] = ctrl.othervalue * ctrl.multiply
      }
    },
  )
